# Autor Martin Medrano Sarmiento
# CBTIS#89
...
def abrir_reportes():
  ...
  # --- Leer archivo ventas.txt y calcular TOTAL ACUMULADO ---
  total_acumulado = 0.0 # Inicializamos el contador
  try:
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
            ...
            for linea in archivo:
                if linea.strip():
                    ...
                    if len(datos) == 4:
                        ...
                        # Sumar el total (el elemento en la posición 3 es el total)
                        try:
                            total_acumulado += float(datos[3])
                        except ValueError:
                            # Manejo de error si el campo total no es un número
                            print(f"Advertencia: El valor de total no es un número en la línea: {linea.strip()}")
                            pass # Continuar con la siguiente línea
  ...
  lbl_total = tk.Label(ventana, 
                         text=f"Total Acumulado de Ventas: ${total_acumulado:,.2f}", # Formato de moneda
                         font=("Arial", 14, "bold"), 
                         bg="#f2f2f2", 
                         fg="green")
...
estilo.configure("TButton", 
                font=("Arial", 12), 
                padding=10,
                background="#0078D7",  # Color azul
                foreground="white")    # Texto blanco

# Configurar el color cuando el botón está presionado
estilo.map("TButton",
